...
const terminalCommands = {
    ...
    skills: () => {
        return skillCategories.map(cat => `${cat.icon} ${cat.title}\n${cat.skills.map(s => `   ${s.name} — ${s.comment}`).join('\n')}`).join('\n\n');
    },
    projects: (args) => {
        ...
        if (list.length === 0)
            return `Nessun progetto nella categoria "${filter}".`;
        return list.map(p => `▸ ${p.title} [${p.category}]\n  ${p.tech.join(' · ')}${p.live ? '\n  ' + p.live : ''}${p.github ? '\n  ' + p.github : ''}`).join('\n\n');
    },
    contact: () => {
        return [
...
// ---------- CAROUSEL ----------
function renderCarousel(images, alt, className) {
    if (images.length === 0)
        return '';
...
function initCarousels() {
    document.querySelectorAll('.carousel').forEach(carousel => {
        if (carousel.dataset.carouselInit === '1')
            return;
...
function initLightbox() {
  ...
  `;
    ...
    document.addEventListener('click', (e) => {
        const btn = e.target.closest('.carousel__expand');
        if (!btn)
            return;
        e.preventDefault();
        e.stopPropagation();
